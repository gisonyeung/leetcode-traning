 ...
 * 1. 方法一中动态修改 map 长度，导致对象访问模式退化到字典模式，访问速度确实会比较慢，速度未必比子串 indexOf 高。
 * 2. 测试用例中或许大部分 case 属于"传入值总长度较长，但不重复子串长度较短"的类型，导致 indexOf 效率反而比 map 高。
 * 3. 上面两点都没说到关键，indexOf 底层使用 C++ 实现，执行效率比 JS 快得多，所以反而暴力法比滑动窗口解法执行时间快，这是语言硬伤。
 */
